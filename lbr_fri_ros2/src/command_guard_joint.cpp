...
CommandGuardJoint::CommandGuardJoint(const CommandGuardParametersJoint &command_guard_parameters)
...
bool CommandGuardJoint::command_in_position_limits_(const_idl_command_t_ref lbr_command,
...
bool CommandGuardJoint::command_in_torque_limits_(const_idl_command_t_ref lbr_command,
                                             const_idl_state_t_ref lbr_state) const {
...
bool SafeStopCommandGuardJoint::command_in_position_limits_(const_idl_command_t_ref lbr_command,
                                                       const_idl_state_t_ref lbr_state) const {
  for (std::size_t i = 0; i < lbr_command.joint_position.size(); ++i) {
...
command_guard_factory(const CommandGuardParametersJoint &command_guard_parameters,
  ...
  if (variant == "default") {
    return std::make_unique<CommandGuardJoint>(command_guard_parameters);
  }
  if (variant == "safe_stop") {
    return std::make_unique<SafeStopCommandGuardJoint>(command_guard_parameters);
  }
